...
public class P59 {
	 ...
	 * then XOR each byte with a given value, taken from a secret key. The advantage with the XOR 
	 * function is that using the same encryption key on the cipher text, restores the plain text; 
	 * for example, 65 XOR 42 = 107, then 107 XOR 42 = 65.
	 * 
	 * For unbreakable encryption, the key is the same length as the plain text message, 
	 * and the key is made up of random bytes. The user would keep the encrypted message and the 
			...
			for (char b ='a';b<='z';b++) {
				...
				for (char c ='a';c<='z';c++) {
					...
					if(r==0){
						...
						int i=0;
						int j=0;
						char ds[] = new char[strs.length];
						while(i<strs.length){
							int d=strs[i]^key[j%3];
							ds[i]=(char) d;
							i++;
							j++;
						}
	...
	public static int decrypt(int strs[],char key[]){
		int count=0;
		int i=0;
		int j=0;
		while(i<strs.length){
			int a=strs[i]^key[j%3];
			//System.out.println(i+" "+j+" "+Character.toLowerCase(a));
			if(!judge((char)a)){
				//System.out.println(strs[i]+" "+i+" "+key[j%3]+" "+(char)a+" "+a);
				return j%3;
			} else {
				count+=a;
			}
