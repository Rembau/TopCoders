...
public class P11 extends Base {
	...
	public static int getDown(int i,int j){
		if(i<=l-4){
	...
	public static int getDownRight(int i,int j){
		if(j<=l-4 && i<=l-4){
			return num[i][j]*num[i+1][j+1]*num[i+2][j+2]*num[i+3][j+3];
		}
		return 0;
	}
	public static int getDownLeft(int i,int j){
		if(j>=3 && i<=l-4){
			return num[i][j]*num[i+1][j-1]*num[i+2][j-2]*num[i+3][j-3];
		}
		return 0;
